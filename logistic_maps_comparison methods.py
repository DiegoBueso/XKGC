...
Created on Wed Dec 21 19:04:47 2020
...
q_ar=np.zeros((nc,ns,nl))
q_kgc=np.zeros((nc,ns,nl))
q_xkgc=np.zeros((nc,ns,nl))
...
# %% RMSE, couling strength and sigma

c=5

fig, (ax1, ax2, ax3) = plt.subplots(1, 3)

ax1.contourf(np.log10(sigma),np.log10(alpha),q_ar[c,:,:])
ax1.grid()

ax2.contourf(np.log10(sigma),np.log10(alpha),q_kgc[c,:,:])
ax2.grid()

ax3.contourf(np.log10(sigma),np.log10(alpha),q_xkgc[c,:,:])
ax3.grid()


# %% plot results
# plt.loglog(sigma,q_ar[10,:],c='k', label='AR')
# plt.loglog(sigma,q_kgc[10,:],c='b', label='KGC')
# plt.loglog(sigma,q_xkgc[10,:],c='r', label='XKGC')
# plt.xlabel('gamma')
# plt.ylabel('RMSE')
# plt.title('c = 0.1')
# plt.grid()
